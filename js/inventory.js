(() => {
  ...
  function initInventoryPage(){
    ...
    function render(){
      const q=(input?.value||'').trim().toLowerCase(); let cars=window.MBStore.getCars().filter(c=>c.active!==false && String(c.brand||'Mercedes-Benz').toLowerCase()==='mercedes-benz' && c.listingUrl).slice();
  ...
  function initVehicleDetail(){
    ...
    const render=()=>{const car=window.MBStore.getCars().find(c=>String(c.id)===String(id)); if(!car){root.innerHTML='<div class="container"><div class="media-empty"><h1>Vehicle not found</h1><p>This listing may have been removed or is no longer available.</p><a class="btn btn-primary" href="/inventory">Back to Inventory</a></div></div>';return;}
      ...
      root.innerHTML=`<section class="vehicle-detail"><div class="container"><a href="/inventory" class="back-link">← Back to Inventory</a><div class="vehicle-detail-grid"><div class="vehicle-detail-media"><img src="${esc(car.image||'images/moses-benz-logo.png')}" alt="${esc(car.name)}" onerror="this.onerror=null;this.src='/images/moses-benz-logo.png'"><span class="vehicle-source-badge">${esc(car.listingSource||'Mercedes-Benz listing')}</span></div><div class="vehicle-detail-copy"><span class="eyebrow">Mercedes-Benz · For Sale</span><h1>${esc(car.name)}</h1><div class="vehicle-price">${window.MBStore.formatNGN(car.priceNGN)}</div><p class="vehicle-description">${esc(car.description||'Contact Moses Benz Auto Care for current availability and full inspection details.')}</p><div class="vehicle-actions"><button class="btn btn-primary" id="vehicle-wa">Chat about this car on WhatsApp</button>${car.listingUrl?`<a class="btn btn-ghost-light" href="${esc(car.listingUrl)}" target="_blank" rel="noopener noreferrer">View original listing</a>`:''}</div></div></div><div class="vehicle-detail-panel"><div><span class="eyebrow">Vehicle details</span><h2>Full specification</h2></div><div class="vehicle-spec-grid">${specs.filter(x=>x[1]!==undefined&&x[1]!==null&&String(x[1]).trim()!=='').map(x=>`<div><span>${esc(x[0])}</span><strong>${esc(x[1])}</strong></div>`).join('')}</div>${features?`<div class="vehicle-features"><h3>Listed features</h3><ul>${features}</ul></div>`:''}<p class="vehicle-note">This page is a preview of the source listing. Availability, price and specifications can change. When you chat with Moses Benz Auto Care, the message will include the vehicle details so the conversation starts with the exact car you viewed.</p></div></div></section>`;
