(() => {
  ...
  function initCatalog(){
    ...
    function cardFor(name,i){const s=slug(name);return `<article class="option-card"><span>${String(i+1).padStart(2,'0')}</span><h3>${esc(name)}</h3><p>${esc(detail[s]||'Mercedes-Benz specialist service covering inspection, diagnosis, repair or maintenance for this system. We explain the finding before work begins.')}</p><a href="/service?service=${encodeURIComponent(name)}" class="text-link page-route">About this service →</a></article>`;}
    function render(){
